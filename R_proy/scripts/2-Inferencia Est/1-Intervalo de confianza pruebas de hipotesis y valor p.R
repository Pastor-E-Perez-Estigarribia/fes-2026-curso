...
attempts = 10000 # Simulamos 1000 muestras 
...
gapdata %>% 
  ...
  facet_grid(year ~ continent, scales = "free") + theme_bw() # optional: add scales="free"
...
p_Histogramas = ggplot(gapdata %>% 
  ...
  facet_grid(year ~ continent, scales = "free")  + theme_bw()
...
gapdata %>%  
  ...
  ggtitle(
    "Life expectancy by continent in 2002 v 2007") + theme_minimal()# add title
...
t.test(paired_table$`2002`,paired_table$`2007`, data = ., paired = TRUE)
...
# 3. vemos dónde se encuentran nuestros datos reales 
